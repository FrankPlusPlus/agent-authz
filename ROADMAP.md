...
- Audit events, signed immutable policy-bundle primitives, local reference
  sinks/stores, tests, a FastAPI example, and distribution checks.
...
- Expand the current operation-coverage manifest with framework-specific
  discovery and generated route inventories.
...
- A persistent reference `PermitStore` and key-rotation/verification-key
  interfaces, documented as reference infrastructure rather than magic
...
- PostgreSQL/Redis reference implementations for permit consumption and audit
  outbox delivery, with retry, retention, and observability guidance.
